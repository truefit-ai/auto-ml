...
class SegmentationModel:
    ...
    def scope(self, dataset, val_dataset, val_metadata,
        ...
        self.full_train_dataloader = get_segmentation_dataloader(
            self.full_train_data, 
                                                       test = False)
        self.train_dataloader = get_segmentation_dataloader(self.train_data, 
                                                       test = False)
        self.val_dataloader = get_segmentation_dataloader(self.val_data, )
                
            
        # assume 5-10it/s; 100 = ~10s  (may be asked for ~360-720s)
        if True:
            start = time.time()
            params = get_segmentation_base_params(self.train_data, 
                                                  self.train_dataloader)
            model = get_segmentation_model(params, 
            ...
            start = time.time()
            model = fit_segmentation(model, params, 
                                self.train_dataloader, self.val_dataloader, );
            self.step_time = (time.time() - start) / params['steps']
            start = time.time()
            yp, y = predict_segmentation(model, self.val_dataloader, limit = 20)
    ...
    def trials(self, params):
        self.param_dict = get_segmentation_param_dict(params, self.full_train_data)
    ...
    def trial(self, params = None):
        ...
        model = get_segmentation_model(model_params,
                    task_type = self.train_dataset.metadata.get_task_type(),
                    final_metric = self.train_dataset.metadata.get_final_metric(),
                    dataset = (self.train_dataset, self.idx_train),
                    dataloader = self.train_dataloader,
                                )
        model, yp, y = fitAndPredict_segmentation(model, params, 
                                      self.train_dataloader,
                                         self.val_dataloader)
        del model
        try:
            score = self.metric(y.reshape(-1), yp.reshape(-1)); 
            print(score); print()
    ...
    def fit(self, params = None):
        if params is None:
            params = get_segmentation_base_params(self.train_data, self.train_dataloader)
        ...
        model = fit_segmentation(model, params, self.full_train_dataloader,)
        return model
    ...
    def test(self, dataset, remaining_time_budget=None):
        ...
        test_dataloader = get_segmentation_dataloader(test_data, )
        yps = []
        for i, model in enumerate(self.models):
            yp, _ = predict_segmentation(model, test_dataloader)        
            yps.append(yp.numpy())
...
def get_segmentation_param_dict(params, data):
    param_dict =  {}
...
#             print('feature norm in-range; not adjusting norm')
        ...
        self.base_freq = nn.Parameter(base_freq.float(),
                requires_grad = (self.params.task_type == 'continous'))
        self.base_norm = nn.Parameter(base_norm.float(),
                requires_grad = (self.params.task_type == 'continous'))
...
#                                          requires_grad = False)
        ...
        self.backbone = smp.Unet(
            encoder_name = 'timm-efficientnet-{}'.format(backbone),        
            # encoder_weights = None,
            in_channels = 3,
            classes = 1,    
        )        
        # self.backbone.encoder.load_state_dict(torch.load(
        ...
        with torch.no_grad():            
            ...
            if base_freq is not None:
                if self.params.task_type == 'single-label':
                    self.backbone.segmentation_head[0].bias[:] = (
                            torch.log(self.base_freq) )
                elif self.params.task_type == 'multi-label':
                    self.backbone.segmentation_head[0].bias[:] = (
                             reverseLogit(self.base_freq) )
                elif self.params.task_type == 'continuous':
                    self.mean_wt = nn.Parameter(torch.ones(n_chs)/n_chs)
                    self.stdev_wt = nn.Parameter(torch.ones(n_chs)/n_chs)
                    
                    
            if n_chs != 3:
                stem = self.backbone.encoder.conv_stem
                stem_wt = stem.weight
                wts = self.backbone.encoder.conv_stem.weight
                self.backbone.encoder.conv_stem = nn.Conv2d(
                    ...
                    bias = stem.bias)
                if n_chs == 1:
                    self.backbone.encoder.conv_stem.weight[:] = (
                    ...
                    self.backbone.encoder.conv_stem.weight[:] += (
                        stem_wt.std() 
                    +   torch.randn(
                        *self.backbone.encoder.conv_stem.weight.shape,
...
#             return x
    ...
    def process(self, xs, ys):
        ...
        ys = ys.cpu().float().unsqueeze(1)
        
        features = []; targets = []; means = []; stdevs = []
        for idx, x in enumerate(xs):
            ...
            c = torch.cat((x, ys[idx],))
            if self.training:
                if random.random() < 1/2:
                    c = transforms.RandomAffine((-15, 15),
                        interpolation = transforms.InterpolationMode.BILINEAR
                    )(c)
                c = transforms.RandomHorizontalFlip()(c)
                
            c = transforms.RandomResizedCrop(
                size = self.img_size[1:],#x.shape[1:],
                scale = (0.3 if self.training else 1.0, 1.0),
                ratio = (0.7, 1.4) if self.training else (1.0, 1.0),
                interpolation = #transforms.InterpolationMode.BICUBIC
                            # if self.img_size[-2:] == (128, 128) else 
                                transforms.InterpolationMode.BILINEAR
                ,
            ...
            min_z = (x.min() - x.mean())/x.std()
            if max_z.item() > 20 or min_z.item() < -20:
                if self.first: print('non-normal')
            ...
            mean = x.reshape(len(x), -1).mean(dim = -1)            
            stdev = x.reshape(len(x), -1).std(dim = -1)            
            x = transforms.Normalize(mean, stdev)(x) * 0.22 + 0.45
            if self.first: print(x.shape, x.mean(), x.std())
            self.first = False
        ...
        y = torch.stack(targets).to(self.device).float()
        means = torch.stack(means).to(self.device).float()
        stdevs = torch.stack(stdevs).to(self.device).float()
        return x, y.reshape(len(y), -1), means, stdevs
    ...
    def forward(self, x, N = 1):
        ...
        with torch.autocast(enabled = False, device_type = 'cuda'):
            if self.params.task_type == 'single-label':
                ...
                yp = yp * self.base_norm + self.base_freq
        ...
        return yp.reshape(len(yp), -1)


    def training_step(self, batch, batch_idx):
        x, y = batch
        with torch.no_grad():
            x, y, means, stdevs = self.process(x, y)            
        # print(x.shape)
        # print(y.shape)
        # with torch.no_grad():
        #     for i in range(len(x)):
        #         print(torch.corrcoef(torch.stack((x[i].flatten(), y[i]))))
        yp = self.forward(x)#, N = 10)
        # yp = yp * torch.prod(stdevs ** self.stdev_wt) + (means * self.mean_wt).sum()
    ...
    def on_validation_epoch_end(self):
        ...
        try:
            if 'label' in self.params.task_type:
                metric = 1 - inv_auroc_score(y.flatten().cpu(), yp.flatten().cpu())
        except Exception as e: metric = np.nan;# print(e); pass;
        self.log('test_auc', metric, 
                     prog_bar = True)
        
        metric = eval(self.params.final_metric)(y.flatten().cpu().numpy(),
                                                yp.flatten().cpu().numpy())
...
def get_segmentation_base_params(train_dataset, train_dataloader):
    # x = train_dataset[0][0]
...
def get_segmentation_stats(train_dataset, idx_train ):
    ...
    targets = np.stack(targets).flatten()
    base_freq = np.mean(targets)
    base_norm = np.std(targets)        
...
def get_segmentation_dataloader(dataset, test = True):
    if not test:
...
def get_segmentation_size(dataloader):
    ...
    print(x_dims)
    x_order = list(range(len(x.shape)))#torch.argsort(x_dims).tolist()
    print(x_order)

    h = min(1024, max(1, x_dims[x_order[-2]].item()))
    # h = (h // 32) * 32
    w = min(1024, max(1, x_dims[x_order[-1]].item()))
    # w = (w // 32) * 32
...
def get_segmentation_model(params, task_type, final_metric, dataset, dataloader):
    base_freq, base_norm = (
        get_segmentation_stats(*dataset))
    
    img_size, x_order = get_segmentation_size(dataloader)
    ...
    model = SegmentationNetwork(**params,
                          img_size = img_size, 
                            x_order = x_order,
...
def fit_segmentation(model, params, train_dataloader, val_dataloader = None, ):
    print(); print(params)
    start = time.time()
...
def fitAndPredict_segmentation(model, params, train_dataloader, val_dataloader, ):
    model = fit_segmentation(model, params, train_dataloader, val_dataloader, );
    yp, y = predict_segmentation(model, val_dataloader)
    return model, yp, y
    
def predict_segmentation(model, test_dataloader, limit = None):
    start = time.time()
    ...
    for batch in test_dataloader:
        x, y = batch
        with torch.no_grad():
            x, y, means, stdevs = model.process(x, y)
            yp = model(x.to(device))
            # yp = (yp * torch.prod(stdevs ** model.stdev_wt) 
            #       + (means * model.mean_wt).sum())
            yps.append(yp)
            ys.append(y)
